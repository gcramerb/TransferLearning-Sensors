...
class networkLight(LightningModule):
	...
	def training_step(self, batch, batch_idx):
		...
		#
		try:
			print(self.model.CNN1[0].weight.grad.nonzero())
		except:
			print(self.model.CNN1[0].weight.grad)
			
		print(loss)
		
		#loss.backward()
		self.compute_grads = True
		opt.step(make_optimizer_step=True)
		tqdm_dict = {"loss": loss}
	...
	#
	# 	sourceIdx = np.where(domain.numpy() == 0)[0]
	# 	targetIdx = np.where(domain.numpy() != 0)[0]
	...
	# 	accSource = accuracy_score(trueSource.numpy(), np.argmax(predSource.numpy(), axis=1))
	# 	accTarget = accuracy_score(trueTarget.numpy(), np.argmax(predTarget.numpy(), axis=1))
	# 	loss = loss.item()
	#
	# 	metrics = {"loss": loss,
